...
# ── 정밀 최적화(최적화 배치 실행) 진행률 표시용 백그라운드 작업 ──────────────────────
# GA 탐색은 몇 초~몇 분(재시작·1차/2차 라운드에 따라)이 걸릴 수 있어, 요청 하나로 끝까지
# 동기 대기하면 화면에 진행 상황을 보여줄 방법이 없다 — /start가 스레드로 탐색을 시작하고
# jobId를 즉시 반환하면, 화면은 /status/<jobId>를 주기적으로 폴링해 진행률을 표시한다.
# 이 도구는 로컬 단일 사용자용이라 별도 큐/DB 없이 프로세스 메모리 딕셔너리로 충분하다.
_optimize_jobs = {}
_jobs_lock = threading.Lock()
JOB_TTL_SECONDS = 15 * 60  # 오래 방치된 완료/오류 작업은 다음 작업 시작 시 정리(무한 누적 방지)


def _cleanup_old_jobs():
    now = time.time()
    with _jobs_lock:
        stale = [jid for jid, job in _optimize_jobs.items() if now - job['startedAt'] > JOB_TTL_SECONDS]
        for jid in stale:
            del _optimize_jobs[jid]


def _run_optimize_job(job_id, envelope, edges, params):
    """백그라운드 스레드 본체 — 스레드 안에서 발생한 예외는 호출자에게 전파되지 않고
    조용히 스레드만 죽으므로(콘솔에 트레이스백만 찍힘), 반드시 여기서 직접 잡아
    status='error'로 기록해야 화면이 무한 폴링하지 않는다."""
    def on_progress(info):
        with _jobs_lock:
            if job_id in _optimize_jobs:
                _optimize_jobs[job_id]['progress'] = info
    try:
        result = massing.optimize_massing(envelope, edges, params, progress_callback=on_progress)
        with _jobs_lock:
            if job_id in _optimize_jobs:
                _optimize_jobs[job_id]['status'] = 'done'
                _optimize_jobs[job_id]['result'] = result
    except Exception as e:
        print('optimize_massing job error:', e)
        with _jobs_lock:
            if job_id in _optimize_jobs:
...
def buildable_envelope():
    ...
    try:
        ...
        if parcel_shape.geom_type == 'MultiPolygon':
            # 다중 필지 선택 시 인접 필지들을 하나의 대지 경계로 합친다. 서로 다른 시점에
            # 조회된 VWorld 필지 경계는 실제로는 맞닿아 있어야 할 변의 좌표가 부동소수점·측량
            # 오차 수준으로 아주 미세하게 어긋나 있는 경우가 흔하다 — 버퍼 없이 unary_union만
            # 하면 이런 경우 실제로는 붙어있는 필지인데도 여러 조각(MultiPolygon)으로 남고,
            # 그 상태로 "가장 큰 조각만 사용"하면 나머지 필지의 면적이 조용히 누락된 채 계산이
            # 진행된다(사용자 피드백: 여러 필지로 구성된 대지에서 앱이 계산한 면적이 실제
            # 등록 면적보다 크게 작게 나옴). 합집합 직전에만 아주 작은 양의 버퍼(1cm)를 줘서
            # 그런 미세한 틈을 메운 뒤 다시 걷어낸다 — 진짜로 떨어져 있는 필지(예: 도로 하나를
            # 사이에 둔 별개 부지)까지 억지로 붙일 만큼 큰 값은 아니다.
            merged = unary_union([g.buffer(0.01) for g in parcel_shape.geoms]).buffer(-0.01)
            if not merged.is_valid:
                merged = merged.buffer(0)
            # 그래도 여러 조각이면(진짜로 떨어져 있는 필지) 어쩔 수 없이 가장 큰 조각만 쓴다 —
            # 이 경우는 애초에 하나의 대지 경계로 볼 수 없다(설계 대상 매스가 서로 떨어진 두
            # 땅에 걸쳐 나뉠 수는 없으므로).
        ...
        for i in range(n):
            ...
            if edge_type is None:
                if length < 1.0:
                    ...
                    edge_type, road_width_m = geo_utils.classify_edge(sample_lon, sample_lat, VWORLD_API_KEY, probe_dir=(nx, ny))
...
@app.route('/api/optimize-massing/start', methods=['POST'])
def optimize_massing_start():
    """/api/optimize-massing과 요청 형식은 동일하지만, GA 탐색을 백그라운드 스레드로 시작하고
    jobId만 즉시 반환한다 — 화면은 /api/optimize-massing/status/<jobId>를 주기적으로 폴링해
    진행률(경과시간·세대·재시작 등)을 표시하고, 완료되면 같은 응답에서 result를 받는다."""
    body = request.get_json()
    if not body or 'buildableEnvelope' not in body:
        return jsonify({'error': 'buildableEnvelope required'}), 400
    _cleanup_old_jobs()
    ...
    thread = threading.Thread(
        ...
        args=(job_id, body['buildableEnvelope'], body.get('envelopeEdges') or [], body),
        daemon=True
    )
    thread.start()
...
@app.route('/api/optimize-massing/status/<job_id>')
def optimize_massing_status(job_id):
    """작업 상태 폴링. 알 수 없는 jobId(서버 재시작·개발서버 리로드로 유실되었거나
    TTL로 정리된 경우 포함)는 404 — 화면은 이걸 "무한 대기"가 아니라 명확한 오류로
    처리해야 한다."""
    with _jobs_lock:
