...
async function queryAndDrawVWorldParcel(lat, lon, kakaoAddress) {
  ...
  // kakaoAddress(검색/역지오코딩으로 얻은 "추정" 주소)는 VWorld 연속지적도(정확한 필지 경계
  // 폴리곤)와 일치한다는 보장이 없다 — Kakao의 지번 좌표는 근사치라, 최근 분할된 필지
  // (예: 604 → 604 + 604-4)의 경계 근처에서는 옆 필지를 가리킬 수 있다(실측 확인됨: "604-4"를
  // 검색해도 실제로는 604 필지가 그려짐). props.jibun은 VWorld가 이 정확한 (lat,lon) 지점에서
  // 실제로 찾아낸 폴리곤 자신의 지번이라 항상 맞다.
  //
  // 단순 부분문자열 포함 검사("...604-4".includes("604"))는 정확히 이 버그 패턴을 놓친다 —
  // "604"가 "604-4"의 접두어라 포함 관계가 성립해버려 불일치를 못 잡는다(실제로 이 로직의
  // 첫 버전이 그랬음 — 604 vs 604-4 사례로 재현·확인). 주소 문자열 끝의 지번 형식 토큰
  // (숫자[-숫자])만 뽑아 정확히 같은지(포함이 아니라 완전일치) 비교해야 한다.
  //
  // props.jibun은 VWorld가 지번 뒤에 지목(地目) 글자를 그대로 붙여서 준다(예: "604-4대" =
  // 지번 604-4 + 지목 "대"[대지]) — 비교 전에 그 지목 접미사를 떼어내지 않으면 같은 필지인데도
  // "604-4" ≠ "604-4대"로 오탐한다(실측 확인됨: 정확히 604-4를 골랐는데도 불일치 경고가 뜸).
  const jibunToken = (addr) => {
    const m = (addr || '').match(/(\d+(-\d+)?)\s*$/);
    return m ? m[1] : null;
  };
  const kakaoJibunToken = jibunToken(kakaoAddress);
  const vworldJibunNumber = (props.jibun || '').replace(/[^0-9-]+$/, ''); // 끝의 지목 글자(대/전/답/임 등) 제거
  const jibunMismatch = !!(vworldJibunNumber && kakaoJibunToken && kakaoJibunToken !== vworldJibunNumber);
  if (jibunMismatch) {
    console.warn(`[필지 불일치] 검색/클릭 주소="${kakaoAddress}"(지번 ${kakaoJibunToken}) 이지만 실제 선택된 필지 지번="${props.jibun}"`);
  }
  const displayAddress = jibunMismatch
    ? `${kakaoAddress} ⚠실제 필지 지번 ${props.jibun}(다른 필지일 수 있음)`
    : (kakaoAddress || props.jibun || '');
...
/**
 * /api/optimize-massing/start를 호출해 백그라운드 작업을 시작하고 jobId를 받는다(GA 탐색
 * 자체는 서버 스레드에서 계속 진행 — 이 호출은 즉시 반환된다). 실패 시 { error }를 반환한다.
 */
async function startOptimizeMassingJob(payload) {
  try {
    const resp = await fetch('/api/optimize-massing/start', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    console.warn('[최적화 배치 작업 시작 실패]', err);
    return { error: err.message || String(err) };
  }
}

/**
 * /api/optimize-massing/status/<jobId>를 폴링한다. jobId를 서버가 모르는 경우(개발서버
 * 리로드로 유실 등) 404가 오는데, 이건 { error, lost: true }로 구분해 반환한다 — 호출부가
 * "아직 실행 중"과 "작업이 사라짐"을 구별해서 처리할 수 있게 한다.
 */
async function fetchOptimizeMassingStatus(jobId) {
  try {
    const resp = await fetch(`/api/optimize-massing/status/${jobId}`);
    if (resp.status === 404) return { error: 'unknown job', lost: true };
    const data = await resp.json();
    if (!resp.ok) return { error: data.error || `HTTP ${resp.status}` };
    return data;
  } catch (err) {
    console.warn('[최적화 배치 진행률 조회 실패]', err);
    return { error: err.message || String(err) };
  }
}
